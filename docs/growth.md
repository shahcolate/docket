...
- [ ] **Publish the pending release to npm.** `package.json` is at 0.5.0 but
  ...
  still installs the previous release. Run `npm publish` from `main`, and
  tag the commit `v0.5.0`: `v0.4.0` is the only git tag that exists today,
  so the release history has holes in it either way.
